...
| 6 — Policy Memory integration | blocked, unsized | Once `Policy_Memory_Implementation_Plan.md`'s work lands and is validated in `experiments/`, port `PolicyMemory` + `context_fusion` into the `src/` Planner's retrieval step, replacing/extending the top-3/top-2/top-3 fusion. Explicitly gated on external completion — do not start or pre-design the fusion logic now. | 1, external (Policy Memory work) |
